...
plan_compute_pipeline(BeamformerComputePlan *cp, BeamformerParameterBlock *pb)
{
	...
	/* NOTE: automatically add HighPassFilter after DAS if DAS is in the pipeline */
	...
	if (has_das && cp->pipeline.shader_count < BeamformerMaxComputeShaderStages) {
		u32 slot = cp->pipeline.shader_count;
		BeamformerShaderDescriptor *sd = cp->shader_descriptors + slot;
		zero_struct(sd);
		...
		cp->pipeline.shaders[slot] = BeamformerShaderKind_HighPassFilter;
		cp->pipeline.parameters[slot] = (BeamformerShaderParameters){0};
		cp->pipeline.shader_count++;
	}
                  ...
                  BeamformerShaderKind shader, u32 shader_slot, BeamformerShaderParameters *sp, Arena arena)
{
	...
	case BeamformerShaderKind_HighPassFilter:{
		...
		GLenum gl_kind = cp->iq_pipeline ? GL_RG32F : GL_R32F;
		glBindImageTexture(0, frame->texture, 0, GL_TRUE, 0, GL_WRITE_ONLY, gl_kind);

		/* NOTE: Bind all 20 frames from circular buffer as input textures */
